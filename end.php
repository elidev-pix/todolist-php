...
$stmt = $pdo->prepare("SELECT start FROM todoliste WHERE todo_id= :todo_id");
...
if ($row && $row['start'] ) {
    ...
    $start= $row['start'];
        ...
        $sql= 'UPDATE todoliste SET end_date = :end_date,end_time = :end_time,end = :end,duration = :duration WHERE todo_id = :todo_id';
