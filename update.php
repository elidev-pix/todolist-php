...
if (isset($_GET['id'])) {
    ...
    $sql = 'SELECT * FROM todoliste WHERE todo_id = :todo_id AND user_id = :user_id';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $todo_name = trim($_POST['todo'] ?? '');

        if ($todo_name !== '') {
            $sql = 'UPDATE todoliste SET todo_name = :todo_name WHERE todo_id = :todo_id AND user_id = :user_id';
            $stmt = $pdo->prepare($sql);
